...
class CallRequestStatus::Approved < CallRequestStatus
  def send_status
    ...
    if sched > twelve_hours
      ...
      CallRequestMailer.delay(run_at: 12.hours.from_now).request_reminder(owner)
    elsif sched < twelve_hours && sched > four_hours
      CallRequestMailer.delay(run_at: 4.houts.from_now).request_reminder(owner)
    elsif sched < four_hours && sched > one_hour
      CallRequestMailer.delay(run_at: 1.hour.from_now).request_reminder(owner)
    else
      CallRequestMailer.request_reminder(owner)
    end
